...
class database:
	def __init__(self,cursor):
		...
		# create admin table:
		query = "CREATE TABLE IF NOT EXISTS `admin` (`id` int PRIMARY KEY,`username` varchar(50),`password` varchar(255));"
		cursor.execute(query)
		cursor.execute("SELECT * FROM admin WHERE username = 'admin'")  
		admin_o=cursor.fetchall()
		if admin_o:
		   pass
		else:
		   cursor.execute("INSERT INTO admin(id, username, password) VALUES (%d,'%s','%s')" % (1,"admin","admin"))         
		...
		query = "CREATE TABLE IF NOT EXISTS `products` (`id` int PRIMARY KEY,`name` varchar(255),`price` float8,`inventory` float8,`kind` varchar(255), `store` varchar(255), `image` varchar(255));"
		cursor.execute(query)

		#create transactions table:
		query= "CREATE TABLE IF NOT EXISTS `transactions` (`order_id` int,`product_id` int,`product_quantity` float8,`date` datetime,`salesperson_id` int,`customer_id` int,PRIMARY KEY (`order_id`, `product_id`));"
		cursor.execute(query)
		...
		query = "ALTER TABLE `products` ADD FOREIGN KEY (`store`) REFERENCES `store` (`store_id`);"
		cursor.execute(query)
